...
def get_user_settings() -> dict[str, Any]:
    ...
    user_settings = {key: value for key, value in all_settings.items() if key not in managed_keys}
...
def main() -> int:
    ...
    if not user_settings:
        log_info(
            "No user-specific settings found (or all are managed by home-manager)",
            out=sys.stdout,
        )
        return 0
