...
def parse_args(argv: Sequence[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument(
        ...
        dest="no_mirror",
        action="store_true",
        help=("Opt out of mirror sync even when rebuilding from a system flake directory."),
    )
    parser.add_argument(
        "--mirror-dir",
        type=Path,
...
def root_ensure_etc_nixos_clone(*, mirror_dir: Path, etc_dir: Path, stderr: TextIO) -> int:
    ...
    if etc_dir.exists():
        ...
        if not (etc_dir / ".git").exists():
            ...
            print(
                f"Refusing to overwrite existing git repo at {etc_dir}. Move it aside first.",
                file=stderr,
            )
            return 1
