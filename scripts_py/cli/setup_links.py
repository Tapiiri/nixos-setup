    ...
    if scripts_dir.is_dir():
        for path in sorted(scripts_dir.iterdir()):
            if path.is_file() and os.access(path, os.X_OK):
                # `rebuild` is a user-facing dispatcher. For local development,
                # `rebuild-dev` should bypass dispatch and run the implementation.
                if path.name == "rebuild":
                    target_name = "rebuild"
                elif path.name == "rebuild-inner":
                    target_name = "rebuild-dev"
                else:
